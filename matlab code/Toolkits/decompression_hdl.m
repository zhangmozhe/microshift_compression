...
while (bitstream_index <= bitstream_length)
    if (mode == 0)
        ...
        % compute gradients: if uniform patch && not first pixel, then go
        % to runlength mode
        if (g1 == g2 && g1 == g3 && g1 == g4 && g1 == 0 && pixel_index > 1)
            mode = 1;
            run_value = b;
        end
    end
    ...
    % predictive mode
    if (mode == 0) % step out and decode Golomb code
        error_value = 0;
...
% % for debugging
% disp(['subimage = ', num2str(subimage_index)])
