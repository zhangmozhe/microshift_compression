function mapped_error_value = error_mapping_hdl(error_value, predict_x, range)
...
upper_error = range - predict_x - int8(1);
if (error_value == 0)
    mapped_error_value = int8(0);
else
    if (position == left) % left
        % mapped to [0, -1, 1, -2, 2, ...]
        if error_value >= 0
            mapped_error_value = min(error_value-int8(1), -lower_error) + error_value;
        else
        ...
        if error_value >= 0 % right
            ...
            mapped_error_value = min(-error_value-int8(1), upper_error) + (-error_value);
        end
    end
end